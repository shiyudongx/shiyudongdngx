...
class MMoE_Model(nn.Module):
    ...
    def forward(self, x):
        ...
        embeddings = torch.cat(input_pair_embedding_list, dim=1)#12*5

        beta = self.beta_layer(x[:, -1].unsqueeze(dim=-1))  #beta_layer()->1,10->20
        x = x[:, :-1]
        input = torch.cat([embeddings, x, beta], dim=1)
        expert_output_list = []
        for i, expert_layer in enumerate(self.expert_list):
            expert_output_list.append(expert_layer(input))  #5 expert output:100
